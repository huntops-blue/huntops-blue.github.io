...
1. Create a dummy network interface to replay traffic across. `echo 'dummy' | sudo tee -a /etc/modules-load.d/dummy.conf && sudo reboot`1. Log in and type `sudo rock setup` to launch the Text User Interface (TUI)
